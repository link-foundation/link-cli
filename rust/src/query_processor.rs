...
impl QueryProcessor {
    ...
    fn preserve_existing_substitution_parts(
        storage: &mut impl NamedTypeLinks,
        pattern: &Pattern,
        solution: &mut HashMap<String, u32>,
        index: u32,
        ...
        if !Self::is_normal_index(index) || !storage.exists(index) {
            return Ok(());
        }
        ...
        if Self::should_preserve_existing_part(pattern.source.as_deref(), solution)
            && Self::can_preserve_existing_part(
                existing_link,
            ...
            )
        {
            *source = existing_link.source;
            Self::assign_variable_from_pattern(pattern.source.as_deref(), *source, solution);
        } else if let Some(bound_source) =
            Self::resolved_variable_part(pattern.source.as_deref(), solution)
        {
            *source = bound_source;
        }

        if Self::should_preserve_existing_part(pattern.target.as_deref(), solution)
            && Self::can_preserve_existing_part(
                existing_link,
            ...
            )
        {
            *target = existing_link.target;
            Self::assign_variable_from_pattern(pattern.target.as_deref(), *target, solution);
        } else if let Some(bound_target) =
            Self::resolved_variable_part(pattern.target.as_deref(), solution)
        {
            *target = bound_target;
        }
    ...
    fn should_preserve_existing_part(
        ...
        pattern.is_some_and(|pattern| {
            pattern.is_leaf()
                && Self::is_variable(&pattern.index)
                && !solution.contains_key(&pattern.index)
        })
    }
    ...
    fn resolved_variable_part(
        ...
        pattern.and_then(|pattern| {
            if pattern.is_leaf() && Self::is_variable(&pattern.index) {
    ...
    fn assign_variable_from_pattern(
        ...
        if let Some(pattern) = pattern {
            Self::assign_variable(&pattern.index, value, solution);
        }
    }
